...
from infer_yolo_v11_seg.infer_yolo_v11_seg_process import InferYoloV11SegParam
...
# --------------------
class InferYoloV11SegWidget(core.CWorkflowTaskWidget):

    def __init__(self, param, parent):
        core.CWorkflowTaskWidget.__init__(self, parent)

        if param is None:
            self.parameters = InferYoloV11SegParam()
        else:
...
# --------------------
class InferYoloV11SegWidgetFactory(dataprocess.CWidgetFactory):

    def __init__(self):
        dataprocess.CWidgetFactory.__init__(self)
        # Set the algorithm name attribute -> it must be the same as the one declared in the algorithm factory class
        self.name = "infer_yolo_v11_seg"

    def create(self, param):
        # Create widget object
        return InferYoloV11SegWidget(param, None)
